(ns clojure-tasks.task4)

; Similar to the task 14 (Java Concurrency) implement a production line for safes and clocks (code skeleton
; with line structure is provided).
; Implementation must use agents for processors (factories) and atoms in combination with agents for
; storages.
; The storage’s agent must be notified by the factory that the appropriate resource is produced, the agent
; puts it to the storage’s atom and notifies all the engaged processors.
...
; Note that there is a shortage for metal. Make sure that both final wares (clocks and safes) are produced
; despite this (at reduced rate of course).
...
(defn notify-msg
  ...
  (try
    ...
    (let [taken (if (zero? (int (* amount 1/2)))
                  1
                  (int (* amount 1/2) )
                )
          notify_amount (state :amount)
          buffer (state :buffer)
          bill (state :bill)
          notify_target @((state :target-storage) :worker)
          changed (assoc state :buffer (assoc buffer ware (+ (buffer ware) taken)))
          internal_buffer (changed :buffer)
          enough-from (fn [map_buf map_ord]
                        (reduce
                          #(and %1 (>= (map_buf %2) (map_ord %2)))
                          true
                          (keys map_ord)
                        )
                      )
      ...
      (do
        (swap! storage-atom #(- % taken))
        (if (enough-from internal_buffer bill)
          (do
            (Thread/sleep (state :duration))
            (supply-msg notify_target notify_amount)
            (assoc state :buffer (reduce
                                   #(assoc %1 %2 (- (internal_buffer %2) (bill %2)))
                                   {}
                                   (keys bill))))
          changed)
        )
...
;;;recompliles the code after it to reset all the process
(defn stop []
  (.stop ore-mine)
  (.stop lumber-mill)
)
